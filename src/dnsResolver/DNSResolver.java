...
public class DNSResolver {
	...
	public static String regExIp = "^(([01]?\\d\\d?|2[0-4]\\d|25[0-5])\\.){3}([01]?\\d\\d?|2[0-4]\\d|25[0-5])$";
	public static String regExHost = "^(([a-zA-Z]|[a-zA-Z][a-zA-Z0-9\\-]*[a-zA-Z0-9])\\.)*([A-Za-z]|[A-Za-z][A-Za-z0-9\\-]*[A-Za-z0-9])$";
	...
	public static List<DNSResponse> GetIpByHostName(String hostName) {
		if (!hostName.matches(regExHost)) {
			System.err.println("The provided string is not a valid URL !");
			return null;
		}
		
		List<DNSResponse> list = null;
		...
		if (null != receiveData) {
			list = new LinkedList<DNSResponse>();
			offset = length + receiveData[length + 1];
			int i = (receiveData[6] << 8) + receiveData[7];
			while (--i > -1) {
				list.add(ExtractResponse(offset, receiveData));
	...
	public static List<DNSResponse> GetHostNameByIp(String ipAddress) {
		if (!ipAddress.matches(regExIp)) {
